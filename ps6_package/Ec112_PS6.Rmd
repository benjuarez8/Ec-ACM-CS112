...
## step 5.
Looking at the previous step, we can see that as the number of books increases (and thus fewer subjects per book), the mean expected mean square errors of mu and tau squared squared decreases while it increases for the theta_j's.  However, we see that these values remain somewhat constant for sigma squared.  Looks like having more books is better since these mean values mu and tau are decreasing.  Would choose 80 books.